    ...
    useEffect(() => {
        ...
        const drawFrame = (deltaSec: number) => {
            ...
            for (let i = 0; i < blobCount; i++) {
                ...
                if (bY[i] > horizonY + bR[i] * 2) {
                    bX[i] = sampleCenterX(w);
                    bY[i] = -10; // Reset above the top edge
                    bVY[i] = 8 + rng() * 28;
                    bR[i] = (1.5 + Math.pow(rng(), 1.8) * 3.5) * worldScale;
                }
                // Progress travel: 0 (top) → 1 (bottom)
                const t = Math.max(0, Math.min(1, bY[i] / denom));
                const fade =
            ...
            for (let i = 0; i < particleCount; i++) {
                ...
                // Progress travel: 0 (top) → 1 (bottom)
                const t = Math.max(0, Math.min(1, pY[i] / denom));
                const fade =
                    t < 0.2 ? t / 0.2 : Math.max(0, 1 - (t - 0.2) / 0.8)
                const a = fade * opacity
                if (a < 0.01) continue
